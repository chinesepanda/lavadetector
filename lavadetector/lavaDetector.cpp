...
//构造与析构函数
lavaDetector::lavaDetector():iPeriod(500)/*检测周期*/,dThreshold(25)/*分割的阈值*/,iRadiusOfCircle_mm(400)/*轴承预设置的半径*/
{
	iNumOfFrames = 0;
	iThickOfWool_p = 0;
}
...
int lavaDetector::getPosOfDetect()//11-21帧确定岩棉检测点、比例尺、熔岩检测点
{
	...
	if(21 == iNumOfFrames)
	{
		...
		for(int i = 0;i<11;i++)
		{
			if(iBiggestDistance[i] == iBiggestDistance_sorted[5])//获取数组中位数对应的index及对应的x坐标
			{
			    ...
			    pPosStreamDetect.y = pPosThickDetect.y - iBiggestDistance_sorted[5];//流股检测点的y坐标等于成纤监测点的y坐标减去轴承纵半径
				break;
			}
		}
	}
