...
int lavaDetector::setPosOfZero_p()//设置零点x坐标
{
...
int lavaDetector::getImageToShow(Mat& imagetoshow)//绘制并获取待显示的检测结果,注意不同模式下显示的图像也不同！
{
	...
	else if (2 == iWorkmode)
	{
		...
		{	
			...
			plotPointTarget(imageToShow_color,pPosThickDown,Scalar(255,0,255),3);//成纤绘制点的坐标
			plotPointTarget(imageToShow_color,pPosThickUp,Scalar(255,0,255),4);//成纤绘制点的坐标
			...
			plotLine(imageToShow_color,pPosStreamDropPlot,Scalar(255,255,255),0);//流股绘制点
			plotPointTarget(imageToShow_color,pPosStreamDropPlot,Scalar(255,0,255),0);//流股绘制点的坐标
...
int lavaDetector::plotPointTarget(Mat& src_color/*待绘制的图像*/,Point pointForPlot,const Scalar& color/*绘制的颜色*/,int modeOfPlot/*绘制方式*/)//在图中绘制点的target
{
	...
	*/
	...
	else if(3 == modeOfPlot)//绘制角向上的三角形
	{
		Point rookPoints[1][3];//定点集
	...
	else if(4 == modeOfPlot)//绘制角向下的三角形
	{
		Point rookPoints[1][3];//定点集
		rookPoints[0][0] = pointForPlot;
		rookPoints[0][1] = Point(pointForPlot.x-30,pointForPlot.y-30);
...
int lavaDetector::detectWidthOfStream()//检测熔岩流股宽度（像素及mm为单位）、检测熔岩流股下落点
{
	...
	*/
	...
	int pre = 100;//前一帧遍历点的亮度
	int current = 100;//当前帧遍历点的亮度
	int i =0;//x坐标参数
	...
	else//如果没有异常值，则直接获取当前帧的成纤厚度参数和成纤绘制点的坐标
	{
		...
		//确定流股落点绘制点的坐标
		int now = 100;//当前像素点的亮度
		int last = 100;//前一个像素点的亮度
		int downY = 0;//下落点y坐标
